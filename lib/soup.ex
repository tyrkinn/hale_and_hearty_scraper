defmodule Soup do
  ...
  This function can only ever return a {:ok, location} tuple because and invalid
  selections will result in this function being recursively called.
  """
  def ask_user_to_select_location(locations) do
    ...
    case IO.gets("Select a location number: ") |> Integer.parse() do
      :error ->
        IO.puts("Invelid selection. Try again.")
  ...
  def fetch_soup_list() do
    case get_saved_location() do
      ...
      _ ->
        IO.puts("It looks like you haven't selected a deafult location. Select one now:")
